...
const LoginModal = ({ onClose }) => {
  ...
  // Function to close modal if click is outside of modal
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div ref={modalRef} className="bg-white p-8 rounded-lg shadow-md w-full max-w-md relative font-sans">
        ...
        <button className="w-full border border-black text-black py-3 rounded-md mt-4 text-center text-base font-bold">Become a member</button>
      </div>
    </div>
  );
};
